...
### Added
- **音声文脈（実験的・オプトイン・既定 OFF・アーカイブ限定）** — 運営が
  文字起こしを用意したアーカイブ動画で、配信の進行文脈（その再生位置までの
  要約＋直近の発言）を AI 判定に反映し、ネタバレ等の判定を補助します。
  **再生位置より先の内容は参照されません**。ON のとき拡張は動画 ID と再生位置
  （秒）を判定リクエストに含めます（OFF のときは送りません）。文字起こしは
  運営側がサーバーで行い、視聴者の端末から音声・字幕を送信することはありません
- **統計パネルに「この配信でのコメント」一覧** — コメント行の ⋯ →
  「📊 統計を見る」で、その視聴者の**この配信内の発言を時刻順に一覧**表示
  （フィルタ済みだけでなく通常コメントも）。単体では判別しにくい煽り・揶揄の
  パターンや、逆に無実であることを視聴者自身が確認できます。**完全に端末内のみ・
  保存されず（配信を離れると消える）・外部送信なし**。この配信の範囲のみで、
  他の配信をまたいだ閲覧はできません
- ポップアップに「音声文脈」タブ（ON/OFF）

### Changed
- **字幕連動（v0.6.0 の実験機能）を廃止** — ゲーム実況の自動字幕は疎で効果が
  限定的だったため終了し、上記「音声文脈」に発展的に置き換えました
- 設定スキーマ v6 へ自動移行（v1〜v5 から後方互換で移行。旧「字幕連動」設定は
  引き継がず、「音声文脈」は一律 OFF で開始します）
- （内部）判定サーバーのレート制限を Cloudflare のネイティブ機構へ移行し、
  バックエンドを強化
