...
**ネタバレ情報を含むなら口調を問わず必ず spoiler を付与する**:
- 指示・助言・攻略アドバイスの口調でも、未到達の情報（敵の属性・正体・分岐・
  入手物・解法・展開）を明かしていれば spoiler。例「次のボスは炎属性だから
  氷装備推奨」＝ネタバレ＋指示 → \`["spoiler","backseat"]\`（spoiler を必ず含める）。
  「次の街でその武器手に入る」「この扉の先に隠しボスいる」「謎解きの答えは
  左右左右」も同様に spoiler を co-tag する
- **進行度・残量・到達点の開示も spoiler**: 「あと2時間でエンディング」
  「ラスボスまであと少し」「もうすぐ最終章」等、ストーリーの残量・進捗の
  開示は未視聴者へのネタバレなので spoiler
- 宣伝や雑談に紛れていても、ネタバレ部分があれば spoiler を含める
  （例「チャンネル登録してね！ところで次のボス炎属性だよ」→ spoiler を含む）
強度判定:
...
2. **複数ラベル可**: 1コメントが複数カテゴリに該当することがある
   ...
   - **spoiler は他カテゴリと排他でない**。ネタバレ性があれば口調・体裁に
     関わらず spoiler を必ず列挙する（backseat / off_topic / spam と co-tag）。
     primary は LABEL_PRECEDENCE が決めるので、モデルは該当ラベルを**漏れなく
     全て**挙げること（spoiler を落として backseat 単独にしない）
